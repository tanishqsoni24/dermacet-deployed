...
class Cart(BaseModel):
    ...
    payment_done_amount_without_offer = models.IntegerField(default=0, null=True, blank=True)
        ...
        for cart_item in cart_items:
            if(cart_item.product.product_available_count > 0):
                if cart_item.quantity > cart_item.product.product_available_count:
                    ...
                    cart_item.save()
                total_price.append((cart_item.product.your_price)*(cart_item.quantity))
    ...
    def set_payment_done_amount_without_offer(self, payment_done_amount_without_offer):
        self.payment_done_amount_without_offer = payment_done_amount_without_offer
        self.save()
...
class CartItems(BaseModel):
    ...
    def get_subtotal(self):
        if self.product is not None:
            return self.product.your_price * self.quantity
        return 0

    def get_paid_subtotal(self):
        if self.product is not None:
...
class MyOrders(BaseModel):
    ...
    tracking_url = models.CharField(max_length=100, null=True, blank=True)
...
def send_email_token(sender, instance, created, **kwargs):
    try:
        if created:
            ...
            Profile.objects.create(user=instance, email=instance.email, email_token=email_token)
