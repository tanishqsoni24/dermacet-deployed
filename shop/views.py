...
def shop(request):
    ...
    top_products = Product.objects.annotate(review_count=Count('reviews')).order_by('-review_count')[:3]
    ...
    if request.GET.get('category') and request.GET.get('sortBy'):
        ...
        if category_fetch == "ALL":
            if sort_factor == "Default":
                products = Product.objects.all()
                return render(request, "shop/index.html", {'products':products,'category':Categories, 'shop':True, 'top_products':top_products})
            if sort_factor == "Popularity":
                products = Product.objects.all().order_by('-buy_count')
                return render(request, "shop/index.html", {'products':products,'category':Categories, 'shop':True, 'top_products':top_products, 'checked_sort':sort_factor})
        ...
        if sort_factor == "Default":
            return render(request, "shop/index.html", {'products':products,'category':Categories, 'shop':True, 'top_products':top_products,'checked_category':category_fetch})
        if sort_factor == "Popularity":
            products = products.order_by('-buy_count')
            return render(request, "shop/index.html", {'products':products,'category':Categories, 'shop':True, 'top_products':top_products, 'checked_category':category_fetch, 'checked_sort':sort_factor})
        if sort_factor == "Price - Low to High":
    ...
    try:
        ...
        if request.method == "POST":
            if request.user.is_authenticated:
                print("enter")
                print(request.POST.get("review_message"))
...
def success(request):
    if request.method == "POST":
        ...
        cost_without_coupon = request.POST.get('cost_without_coupon')
...
def update_discount(request):
    if request.method == "POST":
        ...
        for product in products:
            product.pecentage = discount
            product.your_price = product.price - (product.price * (int(discount)/100))
            product.save()
        messages.success(request, "Discount Updated Successfully")
        return redirect("/shop/update-discount")
    return render(request, "accounts/update_discount.html")